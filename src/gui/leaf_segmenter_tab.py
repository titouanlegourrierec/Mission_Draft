...
class LeafSegmenterTab(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        def validate_dpi(action, value_if_allowed):
            if action == "1":  # insert
        ...
        self.progress = ttk.Progressbar(
            self.progress_frame, orient="horizontal", length=220, mode="determinate"
        )
        ...
        # Zone for stats/graphs
        from src.gui.segmentation_stats_frame import SegmentationStatsFrame

        self.stats_frame = SegmentationStatsFrame(
            self.right_frame, self.output_dir, self.input_dir
        )
    ...
    def browse_model(self):
        path = filedialog.askopenfilename(
            title="Choose Model File", filetypes=[("Model files", "*.*")]
        )
        if path:
    ...
    def run_segmentation(self):
        def process():
            ...
            try:
                ...
                time_format = ""
                if total_elapsed_time >= 3600:  # More than an hour
                ...
                elif total_elapsed_time >= 60:  # More than a minute
    ...
    def _on_output_dir_change(self, *args):
        self._update_stats()

    def _update_stats(self):
        if hasattr(self, "stats_frame"):
            if hasattr(self.stats_frame, "update_stats"):
                self.stats_frame.update_stats()

    def _monitor_progress(self, output_dir, total_images, stop_event):
        """
        Monitoring the output directory to track segmentation progress.
            ...
            try:
                ...
                try:
                    output_files = [
                        ...
                        if (
                            f.lower().endswith((".png", ".tif", ".tiff"))
                            and not f.endswith(".csv")  # noqa
                        )
                    ]
                ...
                if processed_count != last_count:
                    ...
                    self.after(
                        0, lambda t=status_text: self.progress_label.config(text=t)
                    )
